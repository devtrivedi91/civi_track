...
router.get("/history/:uid", async (req, res) => {
  try {
    const snapshot = await getAdminDb().collection("redemptions")
...
router.post("/", async (req, res) => {
  try {
    ...
    await getAdminDb().collection("rewards").doc(rewardId).set(reward, { merge: true });
    res.json({ reward });
  } catch (error) {
...
router.patch("/:id/stock", async (req, res) => {
  try {
    ...
    await getAdminDb().collection("rewards").doc(req.params.id).set({ stock: Number(stock) }, { merge: true });
...
router.post("/redeem", async (req, res) => {
  try {
    ...
    await db.runTransaction(async (transaction) => {
      ...
      transaction.update(userRef, { points: userData.points - rewardData.pointCost });
      transaction.update(rewardRef, { stock: rewardData.stock - 1 });
      
      const prefix = rewardData.title.substring(0,4).toUpperCase().replace(/[^A-Z]/g, "GIFT");
    ...
    res.status(400).json({ error: error.message || "Failed to redeem penalty points." });
  }
});
