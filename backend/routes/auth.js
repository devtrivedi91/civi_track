...
router.post("/login", async (req, res) => {
  try {
    ...
    // Fetch profile
    const userDoc = await getAdminDb().collection("users").doc(cred.user.uid).get();
    ...
    if (userDoc.exists) {
      ...
      // Fallback if missing
      savedProfile = sanitizeUserPayload({
      ...
      await getAdminDb().collection("users").doc(cred.user.uid).set(savedProfile, { merge: true });
    }
    
    res.json({ user: savedProfile, uid: cred.user.uid });
  } catch (error) {
