...
export default function IssueCard({
  ...
  const [activeTab, setActiveTab] = useState<
    "details" | "timeline" | "comments" | "verify" | "ai-solution"
  >("details");
  ...
  const commentsList = issue.timeline
    ...
    .map((event, idx) => {
      let msg = event.note;
      const match =
        event.note.match(/commented:\s*"(.*)"/s) ||
        event.note.match(/:\s*"(.*)"/s);
      if (match) {
        msg = match[1];
      } else {
  ...
  const [aiSolution, setAiSolution] = useState<ResolutionRecommendation | null>(
    null,
  );
  ...
  const isUpvoted = currentUser
    ? issue.upvotes.includes(currentUser.uid)
    : false;

  const handleUpvoteClick = () => {
    if (!currentUser) {
      alert(
        "Please sign in or use a guest account to upvote community issues.",
      );
      return;
    }
  ...
  const handleVerifySubmit = (e: React.FormEvent) => {
    ...
    if (issue.status === "Resolved") {
      onUpdateStatus(
        issue.issueId,
        "Closed",
        "Citizen confirmed resolution: " + verifyNote,
        verifyPhoto || undefined,
      );
    } else {
  ...
  const handleFetchRecommendation = async () => {
    setLoadingAI(true);
    try {
      const response = await fetch(apiUrl("/api/ai/recommend-resolution"), {
        method: "POST",
      ...
      <div className="p-5 border-b border-slate-100">
        <div className="flex flex-wrap items-center justify-between gap-2 mb-2.5">
          <div className="flex items-center gap-2">
            ...
            <span
              className={`text-[11px] font-semibold border px-2.5 py-0.5 rounded-xl ${severityColors[issue.severity]}`}
            >
          ...
          <div className="flex items-center gap-2">
            <span
              className={`text-[11px] font-bold border px-2 py-0.5 rounded-xl uppercase tracking-wide ${statusColors[issue.status]}`}
            >
              {issue.status}
            </span>
            ...
            >
              ...
              >
                <path
                  ...
                  strokeWidth={2}
                  d="M12 4v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"
                />
              </svg>
            </button>
          </div>
      ...
      {showQR && (
        <div className="p-4 bg-slate-50 border-b border-slate-100 flex flex-col items-center text-center">
          <h4 className="font-bold text-xs text-slate-800 mb-1">
            📍 QR Area Reporter Code
          </h4>
          <p className="text-[10px] text-slate-500 mb-3 max-w-xs">
            Scan this QR code with a phone to instantly report an issue or
            review status at this specific landmark.
          </p>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-xs font-medium text-slate-700 leading-normal mt-0.5">
          ...
          <div className="space-y-4">
            ...
            {(currentUser?.role === "Officer" ||
              ...
              <div className="border-t border-slate-100 pt-4 mt-2">
                <h4 className="font-bold text-xs text-indigo-900 mb-2">
                  ⚡ Update Issue Progress (Officer Operations)
                </h4>
              ...
              {commentsList.length === 0 ? (
                ...
                commentsList.map((comment) => (
                  <div
                    ...
                    className="bg-slate-50 p-3 rounded-xl border border-slate-100/60 text-xs leading-relaxed"
                  >
          ...
          <form onSubmit={handleVerifySubmit} className="space-y-3.5">
            {issue.status === "Resolved" ? (
              <div className="bg-emerald-50 text-emerald-900 p-3 rounded-xl border border-emerald-100 text-[11px] leading-relaxed">
                <strong>✅ Confirm Resolution & Close Issue:</strong> Has this
                issue been successfully fixed to your satisfaction? Write a
                confirmation log note (and optionally upload proof). You earn{" "}
                <strong>+10 points</strong> for closing resolved issues!
              </div>
            ) : (
              <div className="bg-indigo-50 text-indigo-900 p-3 rounded-xl border border-indigo-100 text-[11px] leading-relaxed">
                <strong>✊ Community Crowdsourced Validation:</strong> Verifying
                coordinates, uploads, and details accelerates municipal response
                priority. You earn <strong>+5 points</strong> and boost civic
                validation!
              </div>
            )}
            ...
            <div>
              <label className="block text-[10px] font-semibold text-slate-700 mb-1">
                {issue.status === "Resolved"
                  ? "Completion Proof Photo (Optional)"
            ...
            >
              {issue.status === "Resolved"
                ? "Confirm Resolution & Close Issue"
            ...
            {loadingAI ? (
              <div className="flex flex-col items-center justify-center py-8 text-slate-400 space-y-2">
                ...
                <span className="text-[11px]">
                  AI compiling technical guidelines...
                </span>
              </div>
            ) : aiSolution ? (
                ...
                <div className="grid grid-cols-3 gap-2 bg-slate-50 p-2.5 rounded-xl border border-slate-100 text-center text-[10px]">
                  <div>
                    ...
                    <p className="font-bold text-slate-800 mt-0.5">
                      {aiSolution.estimatedHours} Hours
                    </p>
                  </div>
                  <div>
                    <span className="text-slate-400 font-medium">
                ...
                <div className="space-y-1.5">
                  ...
                  <ol className="list-decimal list-inside space-y-1 text-[11px] text-slate-600 leading-normal bg-slate-50/50 p-3 rounded-xl border border-slate-100">
                    {aiSolution.steps?.map((step, idx) => (
                      <li
                        key={idx}
                        className="pl-1 text-slate-700 leading-relaxed"
                      >
                        {step}
                      </li>
                    ))}
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {aiSolution.materials?.map((mat, idx) => (
                      <span
                        key={idx}
                        className="bg-indigo-50 border border-indigo-100/50 text-indigo-700 font-medium text-[10px] px-2.5 py-1 rounded-lg"
                      >
                        {mat}
                      </span>
                    ))}
              ...
              <div className="text-center py-6 text-slate-400 text-xs">
                Unable to retrieve AI guidelines. Click "Re-Generate Plan" to
                retry.
              </div>
            )}
          </div>
